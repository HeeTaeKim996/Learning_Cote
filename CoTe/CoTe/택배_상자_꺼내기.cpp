...
{
    int total = n / w - (num - 1) / w; // ( 맨 위 꼬다리 제외 전체 층 수 ) - ( num 밑의 층 수 )

    int resN = n % w;
    int quotientN = n / w;

    int resNum = num % w;
    int quotientNum = num / w;

    if ((quotientN + quotientNum) % 2 == 0)
    {
        if (resN >= resNum && resN >= 1)
        {
            total++; // 꼬다리
        }
    }
...
int solution(int n, int w, int num)
{
    ...
    while (num <= n)
    {
        count++; // 현재 위치 couunt Up

        num += (((num - 1) / w + 1) * w - num) * 2 + 1;  // 바로 위층으로 올라간다.
    }
