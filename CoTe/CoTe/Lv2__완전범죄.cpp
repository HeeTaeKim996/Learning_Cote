...
{
    // Member : BinaryMax, MinA
    uint64 binaryMax = (static_cast<uint64>(1) << info.size()) - 1;
    int minA = INT32_MAX;

    // Play
    for (uint64 i = 0; i <= binaryMax; i++)
    {
        int a = 0;
        int b = 0;
        ...
        {
            int index = (i >> static_cast<uint64>(j)) & 1;
            if (index == 0)
            {
                a += info[j][0];
                if (a >= n)
    ...
    // ※ 동적 계획법에 대해 알게 된 계기가 된 문제. 위처럼 하면, 성능 문제로 감점되어 40밖에 못받음
}
#else
#include <string>
#include <vector>
...
int solution(vector<vector<int>> info, int n, int m)
{
    vector<int> dp(m, INT32_MAX);
    dp[0] = 0;

    for (const vector<int>& inf : info)
    {
        int aTrace = inf[0];
        int bTrace = inf[1];

        vector<int> next(m, INT32_MAX);

        for (int b = 0; b < m; b++)
        {
            if (dp[b] == INT32_MAX) continue; // 아직 해당 b일 때가 진행되지 않았으므로, continue
...
void main()
{
    ::function<void(vector<vector<int>>, int, int)> TEMP = [](vector<vector<int>> info, int n, int m)
        {
            printf("%d\n", solution(info, n, m));
        };
